...
Page({
  ...
  loadData() {
    const self = this;
    http.post('ACCOUNT_API_INFO_GET_WORK', {}, function(res) {
      wx.hideLoading()
      if (res.code != 200) {
  ...
  onEditButton: function (e) {
    let obj = e.currentTarget.dataset.item;
    wx.navigateTo({ url: '/pages/my/info/workexp/add?id='+obj.id, })
  },
  ...
  remove:function(id){
    let self=this;
    http.post('ACCOUNT_API_INFO_REMOVE_WORK', {id:id}, function (res) {
      wx.hideLoading()
      if (res.code != 200) {
        return wx.showModal({ content: res.err, confirmText: '确定', confirmColor: '#00a7f2', showCancel: false });
      }
