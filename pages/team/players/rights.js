...
Page({
  ...
  bindRightsChange(e) {
    ...
    this.setData({ 'saveData.rights': rights });
  },


  savePlayerInfo() {
    const self = this;
    const rightsId = self.data.saveData.rights.id ? self.data.saveData.rights.id : 0;
    console.log('rightsId:::', rightsId);
    if (rightsId != 2) {
      return self.saveRights(rightsId);
    }
    return wx.showModal({
      content: "确定移交超级管理员权限吗？",
      confirmText: '移交',
      showCancel: true,
      success: function (res) {
        if (!res.confirm) {
